...
const jobQueue = []; // queued job names waiting to train

function checkQueue() {
    if (jobQueue.length > 0 && runningJobs.size === 0) {
        const nextJob = jobQueue.shift();
        console.log(`[Queue] Auto-starting next queued job: ${nextJob}`);
        startTrainingJob(nextJob).catch(err => console.error(`[Queue] Failed to start ${nextJob}:`, err));
    }
}
...
function getGlobalConfig() {
    // Resolve venv path in priority order:
    // 1. ANIMA_VENV_PATH env var (set by start_windows.bat)
    // 2. Auto-detected conda env next to this file (installer_files/Environments/anima)
    // 3. global_config.toml venv_path (only if non-blank)
    // 4. Last resort: venv/ next to server.js
    const condaEnvPath = path.join(__dirname, '..', 'installer_files', 'Environments', 'anima');
    const autoDetected = fs.existsSync(path.join(condaEnvPath, 'python.exe')) ? condaEnvPath : null;
    const envVenvPath = process.env.ANIMA_VENV_PATH || '';
    const resolvedDefault = envVenvPath || autoDetected || path.join(ROOT_DIR, 'venv');

    if (fs.existsSync(GLOBAL_CONFIG_PATH)) {
        try {
            const config = TOML.parse(fs.readFileSync(GLOBAL_CONFIG_PATH, 'utf8'));
            // Ignore blank or stale venv_path in toml — auto-resolve instead
            if (!config.venv_path || config.venv_path.trim() === '' || !fs.existsSync(path.join(config.venv_path, 'python.exe'))) {
                config.venv_path = resolvedDefault;
            }
            return config;
        } catch (err) {
        ...
        const jobs = fs.readdirSync(JOBS_DIR, { withFileTypes: true })
            ...
            .map(d => {
                ...
                return {
                    ...
                    queued: jobQueue.includes(d.name),
                    mtime
                };
            })
            .sort((a, b) => b.mtime - a.mtime);
...
app.get('/api/jobs/:name/prompts', (req, res) => {
    try {
        const jobPath = getJobPath(req.params.name);
        const uiPromptsPath = path.join(jobPath, 'ui_prompts.json');
        const promptsPath = path.join(jobPath, 'sample_prompts.txt');

        if (fs.existsSync(uiPromptsPath)) {
            const prompts = JSON.parse(fs.readFileSync(uiPromptsPath, 'utf8'));
        ...
        prompts = prompts.map(p => p.replace(/\s*--[a-z]\s+[^\s]+/g, '').replace(/\s*--n\s+"[^"]*"/g, '').trim());
        res.json({ prompts });
    } catch (err) {
        res.status(500).json({ error: err.message });
    }
...
app.put('/api/jobs/:name/prompts', (req, res) => {
    try {
        ...
        const compiledPrompts = req.body.compiledPrompts || prompts;

        // Strip quotes from --n values — line_to_prompt_dict captures everything after --n as-is
        // so quotes become literal characters and break the tokenizer
        const sanitizedPrompts = compiledPrompts.map(line =>
            line.replace(/--n\s+"([^"]*)"/g, '--n $1')
                .replace(/--n\s+'([^']*)'/g, '--n $1')
        );

        fs.writeFileSync(uiPromptsPath, JSON.stringify(prompts, null, 2), 'utf8');
        fs.writeFileSync(promptsPath, sanitizedPrompts.join('\n') + (sanitizedPrompts.length ? '\n' : ''), 'utf8');
        res.json({ success: true });
    } catch (err) {
        res.status(500).json({ error: err.message });
    }
...
function getVenvPaths(venvPath) {
    if (isWindows) {
        // Detect whether this is a conda env (python.exe at root) or a standard venv (python.exe in Scripts/)
        const condaPython = path.join(venvPath, 'python.exe');
        const venvPython  = path.join(venvPath, 'Scripts', 'python.exe');
        const isConda = fs.existsSync(condaPython) && !fs.existsSync(path.join(venvPath, 'pyvenv.cfg'));
...
function buildShellScript(activatePathOrVenv, envVars, command) {
    if (isWindows) {
        // activatePathOrVenv may be a venv object (conda) or a plain path string (standard venv)
        if (activatePathOrVenv && typeof activatePathOrVenv === 'object' && activatePathOrVenv.conda) {
            // Conda env: prepend env paths to $env:PATH, no Activate.ps1 needed
            const envPath = activatePathOrVenv.envPath;
            const condaSetup = `$env:PATH = '${envPath}\\Scripts;${envPath};${envPath}\\Library\\bin;' + $env:PATH;`;
            return `${condaSetup}\n${envVars}\n${command}`;
        }
        // Standard venv: use Activate.ps1
        const activatePath = typeof activatePathOrVenv === 'object' ? activatePathOrVenv.activate : activatePathOrVenv;
        return `& "${activatePath}";\n${envVars}\n${command}`;
    } else {
        const activatePath = typeof activatePathOrVenv === 'object' ? activatePathOrVenv.activate : activatePathOrVenv;
...
app.post('/api/jobs/:name/train/stop', async (req, res) => {
    try {
        ...
        // Check if job is queued first
        const qIdx = jobQueue.indexOf(jobName);
        if (qIdx > -1) {
    ...
    try {
        ...
        if (keepLoaded) {
            ...
            if (!persistentGenProcess) {
                ...
                const script = buildShellScript(venv, envVars, launchCmd);
                ...
                // Respond immediately - don't block the HTTP request while model loads
                res.json({ success: true, message: "Persistent generation server starting. Watch the console log for progress." });

                // Wait for server in background (up to 120s for large models)
                (async () => {
                    let attempts = 0;
                    while (attempts < 120) {
                        ...
                        try {
                            ...
                            if (ping.ok) {
                                if (persistentGenProcess) persistentGenProcess.ready = true;
                                broadcastLog(jobName, `\n[Gen Server] Ready on port ${port} — click Generate to create images\n`);
                                break;
                            }
                        } catch (e) { }
                        attempts++;
                    ...
                    if (attempts >= 120) {
                        broadcastLog(jobName, `\n[Gen Server] ERROR: timed out waiting for server to start\n`);
                        killPersistentGen();
                    }
                })();
                return;
            ...
            // Server already running but model may still be loading
            if (!persistentGenProcess.ready) {
                return res.status(202).json({ success: false, error: 'Model is still loading — watch the console log and click Generate again when you see [Gen Server] Ready.' });
            }

            // Server already running - send generation request directly
            const payload = {
                sample_prompts: promptsPath,
            ...
            const oneShotScript = buildShellScript(venv, oneShotEnvVars, oneShotCmd);
            ...
            // Free GPU from any persistent gen server before training starts
        if (persistentGenProcess) {
            console.log('[Train] Unloading persistent gen server to free GPU for training...');
            killPersistentGen();
        }
...
app.post('/api/jobs/:name/train/start', async (req, res) => {
    try {
        ...
        if (runningJobs.has(jobName) || jobQueue.includes(jobName)) {
            return res.status(400).json({ error: 'Job already active or queued' });
        }

        // If another training job is running, queue this one
        const hasTrainingJob = Array.from(runningJobs.values()).some(j => j.type === 'training');
        if (hasTrainingJob) {
        ...
        const trainScript = buildShellScript(venv, trainEnvVars, trainCmd);
...
app.get('/api/jobs/:name/train/status', (req, res) => {
    try {
        ...
        const isQueued = jobQueue.includes(jobName);
        res.json({ running: isRunning, queued: isQueued });
    } catch (err) {
        res.status(500).json({ error: err.message });
    }
});
...
app.post('/api/jobs/:name/tensorboard', async (req, res) => {
    try {
        ...
        if (tbProcesses.has(jobName)) {
            const tb = tbProcesses.get(jobName);
            if (!tb.ready) {
                return res.status(202).json({ success: false, error: 'TensorBoard is still starting up, please wait a moment and try again.' });
            }
        ...
        const tbScript = buildShellScript(venv, '', tbCmd);
        ...
        tbProcesses.set(jobName, { process: proc, port, pid: proc.pid, ready: false });

        // Wait for TensorBoard to be ready before returning the URL
        // TB writes to stderr when ready so we ping the HTTP endpoint
        let ready = false;
        for (let i = 0; i < 30; i++) {
            await new Promise(r => setTimeout(r, 1000));
            try {
                ...
                if (ping.ok || ping.status === 200) {
                    ready = true;
                    if (tbProcesses.has(jobName)) tbProcesses.get(jobName).ready = true;
                    break;
                }
            } catch (e) { /* not up yet */ }
        }
        ...
        if (!ready) {
            tbProcesses.delete(jobName);
            nextTbPort--; // reclaim the port so it doesn't keep incrementing on failure
            try { proc.kill(); } catch(e) {}
            return res.status(500).json({ error: 'TensorBoard failed to start within 30 seconds. Check the server console for errors.' });
        }
